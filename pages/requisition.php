...
function rqRenderSessionTabs() {
    ...
    // Meal types with no order yet today — dashed "+ start" chips (no phantom drafts pre-created).
    (rqTypes || []).filter(t => !mealGroups[t.code]).forEach(t => {
        html += `<button onclick="rqStartMeal('${escHtml(t.code)}')" title="Start a ${escHtml(rqTypeName(t.code))} order"
            class="text-xs font-medium px-3 py-1.5 rounded-full border border-dashed border-gray-300 text-gray-400 hover:text-orange-500 hover:border-orange-300 whitespace-nowrap transition">
            + ${escHtml(rqTypeName(t.code))}
        </button>`;
    });
...
// Start a meal type that has no order yet — creates its draft on demand, then reloads.
async function rqStartMeal(mealCode) {
    try {
        await api('api/requisitions.php?action=ensure_session', {
            method: 'POST',
            body: JSON.stringify({ kitchen_id: RQ_KITCHEN_ID, req_date: rqDate, meals: mealCode, guest_count: rqGuestCount })
        });
        await rqInit();
        const grp = rqSessions.filter(s => s.meals === mealCode);
        if (grp.length) rqLoadSession(grp[0].id);
    } catch (e) {
...
async function rqAddDish(recipeId) {
    ...
    try {
        ...
        // Breakfast allows simple/no-recipe items (toast, fruit, etc.); other meals need ingredients.
        if (ingredients.length === 0 && (rqActiveSession?.meals !== 'breakfast')) {
...
function rqRenderSelectedDishes(isDraft) {
    ...
    dishList.forEach(d => {
        ...
        const scaledTotal = d.ingredients.filter(i => !(i.is_primary !== undefined && parseInt(i.is_primary) === 0)).length;
