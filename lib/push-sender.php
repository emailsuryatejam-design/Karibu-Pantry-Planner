<?php
/**
 * Karibu Pantry Planner — Web Push Sender
 * Pure PHP implementation using VAPID + curl (no Composer dependencies)
 */
...
function sendPushNotification(array $subscription, array $payload): array {
    ...
    // 201 = success, 410 = subscription expired (should remove)
    return [
...
function derToRaw(string $der): ?string {
    ...
    // Ensure 32 bytes each, strip leading zeros
...
function createEcPem(string $privateKeyRaw): ?string {
    // Build DER for EC private key (prime256v1 OID = 1.2.840.10045.3.1.7)
    $oid = hex2bin('06082A8648CE3D030107');
    $key = "\x04\x20" . str_pad($privateKeyRaw, 32, "\0", STR_PAD_LEFT);
    $seq = "\x30" . chr(2 + strlen($key) + 2 + strlen($oid))
         . "\x02\x01\x01" // version
...
function createEcPublicPem(string $x, string $y): ?string {
    ...
    // Algorithm identifier: ecPublicKey + prime256v1
    $oid1 = hex2bin('06072A8648CE3D0201'); // ecPublicKey
    $oid2 = hex2bin('06082A8648CE3D030107'); // prime256v1
